...
<body onload='rotate()'>
	...
	<% String stock_code = (String) request.getAttribute("stock_code");
				System.out.println("da: " + stock_code); // 종목코드 테스트%>
	<script>
		var cd = stock_code;
		// 차트 부분만 reload
		var repeatChart = function() {
			myAjax("/Stock_Insigh/csv", "code=" + cd, function() {
				ajaxMakeChart_kospi(right_final, this.responseText.trim()); //데이터, 그리기 함수가 들어간 함수
			});
		}
		// 현재 가격 부분만 reload
		var loadPresentPrice = function() {
			...
			myAjax("/Stock_Insigh/csv", "code=" + cd, function() {
				currentData_kospi(stock_pre_data, this.responseText.trim()); //현재가격 영역
			});
		};
		// 차트 그리기
	...
	<section id="content">
		...
		<div class="final_content">
			<div class="left_final">
				...
				<p style="font-size: 35px; color: black;">
					<b>${selectField}</b>분야 :
				</p>
				<!-- yh 사용 -->
               ...
               %>
				...
				<div class="realtime_price">
					<b>${selectCompany}</b>의 실시간 가격 <br> <b
						style="font-size: 20px;">330,600원</b>입니다.
				</div>
				<div class="price">
					내일 예측 가격은<br> <b
			...
			<div id="right_final"
				style="border: 1px solid black; float: left; margin-left: 5%; width: 30%; height: 450px;">
				<!-- chat -->
				실시간 차트
			</div>
		</div>
